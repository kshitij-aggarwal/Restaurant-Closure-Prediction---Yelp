...
# Loading Libraries #
#####################
```{r Package Install, message=FALSE, warning=FALSE, paged.print=FALSE}
...
#install.packages("kernlab") (This package is required)
#library(kernlab) (This package is required)
packages <- c("devtools","rlang","caret","ggplot2","plyr","dplyr","e1071","mlr","randomForest","sqldf","rpart","corrplot","pROC",
              "gmodels","adabag","class","caTools","ROCR","zipcode","reshape2","tidyverse","maps","viridis","albersusa","kernlab",
              "RColorBrewer","ModelMetrics")
...
```{r Filtering Data in Business Hours File}
...
```{r Aligning all files with the zipcode package}
data("zipcode")
...
#Averaging Population from 2007 to 2016
```{r Averaging Population}
...
```{r Exploratory Analysis - Business}
str(business)
...
#Summarized Categories (Top 10 Business Categories)
```{r Exploratory Analysis - Business - Top 10 Business Categories}
...
```{r Exploratory Analysis - Business - Top 20 Cities with Restaurants}
...
```{r Exploratory Analysis - Business - Restaurants by Star Rating}
...
#Aggregating number of restaurants by Postal Code
```{r Exploratory Analysis - Business - Restaurants by Postal Code Aggregation}
...
```{r Exploratory Analysis - Business - US geographical map}
us <- map_data('state')
...
```{r Exploratory Analysis - Review - Aggregations}
...
#We will be removing the ids "DN0b4Un8--Uf6SEWLeh0UA" and "RESDUcs7fIiihp38-d6_6g" as they have a substantial higher reactions as compared to the other restaurants.
```{r Exploratory Analysis - Review - Filtering outliers}
review <- review %>% filter(review$reactions < 16650)
```

#Top 10 Restaurants by reviews
```{r Exploratory Analysis - Review - Top 10 Restaurants by reviews}
...
```{r Exploratory Analysis - Review - Top 10 Restaurants by Reactions}
...
ggplot(data=review_reactions, aes(x=factor(name, levels = name[order(count)]),y=count,fill=count,label = count)) +
  ...
  labs(x="Restaurants",y="Count of Reactions", title="Top 10 Restaurants by Reactions")+
  theme(plot.title = element_text(hjust = 0.5))
```

#Star Rating Histogram
```{r Exploratory Analysis - Review - Histogram - Star Rating}
...
```{r Exploratory Analysis - Review - Histogram - Age of Reviews}
...
```{r Exploratory Analysis - Business Hours}
str(business_hrs)
summary(business_hrs)
```

#Average hours of operation day wise
```{r Exploratory Analysis - Business Hours - Avg Operational Hours by Day}
...
#Assigning dummy code to consider if the restaurant is open on Weekends, Weekdays and sundays
```{r Exploratory Analysis - Business Hours - Adding features}
...
```{r Data Consolidation - Aggregating Reaction}
...
```{r Correlation/Collinearity Analysis}
...
#--------------------------------------------------------------------------------
# Removing unnecassary objects
#--------------------------------------------------------------------------------
```{r Removing unnecassary objects}
rm(business)
rm(business_cat)
rm(business_city)
rm(business_hrs)
...
```{r Model Preperation - Data Spliting}
set.seed(123)
...
#Normalization
```{r Model Preperation - Data Normalization}
...
```{r Model Preperation - Principal Component Analysis}
...
```{r Model Preperation - Principal Component Analysis Filtering}
...
#Removing is_sunday
```{r Model Construction - Logistic Regression - Variable Elimination - is_sunday}
...
#Removing postal_res
```{r Model Construction - Logistic Regression - Variable Elimination - postal_res}
...
#Removing is_weekday
```{r Model Construction - Logistic Regression - Variable Elimination - is_weekday}
log_model <- glm(is_open~review_count+Stars+review_age_wks+review_per_wk+reactions_per_wk+reactions+
...
#Removing is_weekend
```{r Model Construction - Logistic Regression - Variable Elimination - is_weekend}
log_model <- glm(is_open~review_count+Stars+review_age_wks+review_per_wk+reactions_per_wk+reactions+
...
```{r Model Construction - Logistic Regression - Variable Elimination - reactions}
...
#Train Accuracy
```{r Model Construction - Logistic Regression - Train Accuracy}
...
#Validation Accuracy
```{r Model Construction - Logistic Regression - Validation Accuracy}
...
#ROC
```{r Model Construction - Logistic Regression - ROC Curve}
...
```{r Model Construction - Logistic Regression - AUC}
auc(log_model)
```
...
#With PCA
#Accuracy: 71.71%
```{r Model Construction - Logistic Regression - With PCA}
log_model_pca_raw <- glm(is_open~., data=train_set_pca,  family=binomial)
summary(log_model_pca_raw)
```

#Removing PC2
```{r Model Construction - Logistic Regression - With PCA - Variable Elimination - PC5}
log_model_pca <- glm(is_open~PC1+PC3+PC4+PC5+PC6, data=train_set_pca,  family=binomial)
summary(log_model_pca)
```

#Removing PC5
```{r Model Construction - Logistic Regression - With PCA - Variable Elimination - PC4}
log_model_pca <- glm(is_open~PC1+PC3+PC4+PC6, data=train_set_pca,  family=binomial)
summary(log_model_pca)
```

#Validation Accuracy
```{r Model Construction - Logistic Regression - With PCA - Validation Accuracy}
...
#ROC
```{r Model Construction - Logistic Regression - With PCA - ROC Curve}
...
#Here we will be tuning the model by varying the value mtry. The value for trees (ntree) of 50 and 500 did not give a #very big differnce in AUC and Accuracy, so 50 has been taken for computional ease.
```{r Model Construction - Random Forest}
set.seed(123)
...
#ROC
```{r Model Construction - Random Forest - ROC Curve}
roc_rf <- roc(val_set$is_open,predValid)
plot(roc_rf)
```

#AUC
#We can label this a an fair classisfier but its doing a fair job in the classification with AUC of 0.59
```{r Model Construction - Random Forest - AUC}
roc_rf
```

#With PCA, optimal mtry was 2 for the pca data set as well
#Accuracy: 72.74%
```{r Model Construction - Random Forest - With PCA}
set.seed(123)
...
#ROC
```{r Model Construction - Random Forest - With PCA - ROC Curve}
...
#rbfdot
```{r Model Construction - SVM - rbfdot}
set.seed(123)
...
#polydot
```{r Model Construction - SVM - polydot}
set.seed(123)
...
#laplacedot
```{r Model Construction - SVM - laplacedot}
set.seed(123)
sv_ld <- ksvm(is_open ~ ., data = train_set,kernel = "laplacedot",type="nu-svc",prob.model = TRUE)

pred_svm_ld <- predict(sv_ld, val_set)
caret::confusionMatrix(data = as.factor(pred_svm_ld), reference = as.factor(val_set$is_open))
```

#ROC
#We cannot label this a an excellent classisfier but classify a pat of the data with AUC of 0.56
```{r Model Construction - SVM - laplacedot - ROC Curve}
roc_svm <- roc(val_set$is_open,pred_svm_ld)
plot(roc_svm)
```

#AUC
```{r Model Construction - SVM - laplacedot - AUC}
roc_svm
```
...
#rbfdot
```{r Model Construction - SVM - With PCA - rbfdot}
set.seed(123)
...
#polydot
```{r Model Construction - SVM - With PCA - polydot}
set.seed(123)
...
#laplacedot
```{r Model Construction - SVM - With PCA - laplacedot}
set.seed(123)
...
```{r Model Construction - SVM - With Bagging}
# set.seed(123)
...
#Train Accuracy: 74.4%
```{r Model Construction - Bagging - With Cross Validation}
...
```{r Model Construction - Ensemble Model}
...
```{r Model Construction - Ensemble Model - Top Layer as Bagged AdaBoost}
